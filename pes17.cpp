...
void fill_player_entry17(player_entry &players, int &current_byte, void* ghdescriptor)
{
	...
	players.kick_pwr = pDescriptorOld->data[current_byte] >> 6;
	current_byte++;
	players.kick_pwr += (pDescriptorOld->data[current_byte] << 2) & 127;

	players.exp_pwr = pDescriptorOld->data[current_byte] >> 5;
	current_byte++;
	players.exp_pwr += (pDescriptorOld->data[current_byte] << 3) & 127;
	...
	players.reg_pos = pDescriptorOld->data[current_byte] >> 6;
	current_byte++;
	players.reg_pos += (pDescriptorOld->data[current_byte] << 2) & 12;
	
	//Unknown C - 1 bit

//	if(players.id == 73301)
//	{
//		int test1 = pDescriptorOld->data[current_byte] >> 3;
//		test1 = 0;
//	}

	players.play_style = pDescriptorOld->data[current_byte] >> 3;
	current_byte++;

	players.ball_ctrl = pDescriptorOld->data[current_byte] & 127;

	players.ball_win = pDescriptorOld->data[current_byte] >> 7;
	current_byte++;
	players.ball_win += (pDescriptorOld->data[current_byte] << 1) & 127;
	...
	current_byte++;
	players.mo_armr += (pDescriptorOld->data[current_byte] << 1) & 3;

	players.mo_ck = (pDescriptorOld->data[current_byte] >> 2) & 7;
...
//	}
	...
	players.cover = pDescriptorOld->data[current_byte] >> 5;
	current_byte++;
	players.cover += (pDescriptorOld->data[current_byte] << 3) & 127;

	players.weak_use = (pDescriptorOld->data[current_byte] >> 4) & 3;

	players.play_pos[0] = (pDescriptorOld->data[current_byte] >> 6) & 3;
	current_byte++;
	players.play_pos[1] = pDescriptorOld->data[current_byte] & 3;
	players.play_pos[2] = (pDescriptorOld->data[current_byte] >> 2) & 3;
	players.play_pos[3] = (pDescriptorOld->data[current_byte] >> 4) & 3;
	players.play_pos[4] = (pDescriptorOld->data[current_byte] >> 6) & 3;
	current_byte++;
	players.play_pos[5] = pDescriptorOld->data[current_byte] & 3;
	players.play_pos[6] = (pDescriptorOld->data[current_byte] >> 2) & 3;
	players.play_pos[7] = (pDescriptorOld->data[current_byte] >> 4) & 3;
	players.play_pos[8] = (pDescriptorOld->data[current_byte] >> 6) & 3;
	current_byte++;
	players.play_pos[9] = pDescriptorOld->data[current_byte] & 3;
	players.play_pos[10] = (pDescriptorOld->data[current_byte] >> 2) & 3;
	players.play_pos[11] = (pDescriptorOld->data[current_byte] >> 4) & 3;
	players.play_pos[12] = (pDescriptorOld->data[current_byte] >> 6) & 3;
	current_byte++;

	players.mo_hunchd = pDescriptorOld->data[current_byte] & 3;

	players.mo_hunchr = (pDescriptorOld->data[current_byte] >> 2) & 3;

	players.mo_pk = (pDescriptorOld->data[current_byte] >> 4) & 3;

	players.place_kick = pDescriptorOld->data[current_byte] >> 6;
	current_byte++;
	players.place_kick += (pDescriptorOld->data[current_byte] << 2) & 124;

	players.b_edit_playpos = (pDescriptorOld->data[current_byte] >> 5) & 1;
	...
	players.speed = pDescriptorOld->data[current_byte] >> 7;
	current_byte++;
	players.speed += (pDescriptorOld->data[current_byte] << 1) & 127;

	players.b_edit_style = (pDescriptorOld->data[current_byte] >> 6) & 1;

	players.b_edit_com = pDescriptorOld->data[current_byte] >> 7;
	current_byte++;

	players.b_edit_motion = pDescriptorOld->data[current_byte] & 1;

	players.b_base_copy = (pDescriptorOld->data[current_byte] >> 1) & 1;

	//Unknown D - 1/1
	
	players.strong_foot = (pDescriptorOld->data[current_byte] >> 3) & 1;
	//Unknown E - 1/1
...
void fill_team_ids17(team_entry &teams, int &current_byte, void* ghdescriptor)
{
	...
	current_byte+=0xE; //12
	teams.color1_red = (pDescriptorOld->data[current_byte]) & 63;

	teams.color1_green = (pDescriptorOld->data[current_byte] >> 6);
	current_byte++; //13
	teams.color1_green += (pDescriptorOld->data[current_byte] << 2) & 63;
	
	current_byte++; //14
	teams.color2_red = (pDescriptorOld->data[current_byte]) & 63;
	
	teams.color2_green = (pDescriptorOld->data[current_byte] >> 6);
	current_byte++; //15
	teams.color2_green += (pDescriptorOld->data[current_byte] << 2) & 63;
	
	teams.color2_blue = (pDescriptorOld->data[current_byte] >> 4);
	current_byte++; //16
	teams.color2_blue += (pDescriptorOld->data[current_byte] << 4) & 63;

	teams.color1_blue = (pDescriptorOld->data[current_byte] >> 2);
	

	//current_byte+=0x94;
	current_byte+=0x3; //19
	teams.b_edit_name = (pDescriptorOld->data[current_byte] >> 4) & 1;
...
//	unsigned char test[4];
	...
	pDescriptorOld->data[current_byte] += player.kick_pwr << 6; //2/7
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.kick_pwr >> 2) & 31; //5/7

	pDescriptorOld->data[current_byte] += player.exp_pwr << 5; //3/7
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.exp_pwr >> 3) & 15; //4/7
	...
	pDescriptorOld->data[current_byte] = player.age & 63; //6/6
	
	pDescriptorOld->data[current_byte] += player.reg_pos << 6; //2/4
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.reg_pos >> 2) & 3; //2/4

	//Unknown C - 1/1
	pDescriptorOld->data[current_byte] += (player.play_style << 3); //5/5
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.ball_ctrl & 127; //7/7
	
	pDescriptorOld->data[current_byte] += player.ball_win << 7; //1/7
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.ball_win >> 1) & 63; //6/7
	
	pDescriptorOld->data[current_byte] += player.weak_acc << 6; //2/2
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.jump & 127; //7/7
	
	pDescriptorOld->data[current_byte] += player.mo_armr << 7; //1/3
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.mo_armr >> 1) & 3; //2/3
	
	pDescriptorOld->data[current_byte] += (player.mo_ck << 2) & 12; //3/3
	
	pDescriptorOld->data[current_byte] += (player.cover << 5) & 224; //3/7
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.cover >> 3) & 15; //4/7

	pDescriptorOld->data[current_byte] += (player.weak_use << 4) & 48; //2/2
	
	pDescriptorOld->data[current_byte] += (player.play_pos[0] << 6); //2/2
	current_byte++;
	pDescriptorOld->data[current_byte] = player.play_pos[1]; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[2] << 2; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[3] << 4; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[4] << 6; //2/2
	current_byte++;
	pDescriptorOld->data[current_byte] = player.play_pos[5]; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[6] << 2; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[7] << 4; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[8] << 6; //2/2
	current_byte++;
	pDescriptorOld->data[current_byte] = player.play_pos[9]; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[10] << 2; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[11] << 4; //2/2
	pDescriptorOld->data[current_byte] += player.play_pos[12] << 6; //2/2
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.mo_hunchd; //2/2
	
	pDescriptorOld->data[current_byte] += player.mo_hunchr << 2; //2/2
	
	pDescriptorOld->data[current_byte] += player.mo_pk << 4; //2/2
	
	pDescriptorOld->data[current_byte] += (player.place_kick << 6) & 192; //2/7	
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.place_kick >> 2); //5/7
	
	pDescriptorOld->data[current_byte] += player.b_edit_playpos << 5; //1/1
	...
	pDescriptorOld->data[current_byte] = player.stamina & 127; //7/7
	
	pDescriptorOld->data[current_byte] += player.speed << 7; //1/7
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.speed >> 1) & 63; //6/7
	
	pDescriptorOld->data[current_byte] += player.b_edit_style << 6; //1/1
	
	pDescriptorOld->data[current_byte] += player.b_edit_com << 7; //1/1
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.b_edit_motion; //1/1
	
	pDescriptorOld->data[current_byte] += player.b_base_copy << 1; //1/1
	
	//Unknown D - 1/1
	pDescriptorOld->data[current_byte] += player.strong_foot << 3; //1/1
	
	//Unknown E - 1/1
	pDescriptorOld->data[current_byte] += player.com_style[0] << 5; //1/1
	pDescriptorOld->data[current_byte] += player.com_style[1] << 6; //1/1
	pDescriptorOld->data[current_byte] += player.com_style[2] << 7; //1/1
	current_byte++;
	...
	pDescriptorOld->data[current_byte] = player.b_edit_face; //1/1
	pDescriptorOld->data[current_byte] += player.b_edit_hair << 1; //1/1
	pDescriptorOld->data[current_byte] += player.b_edit_phys << 2; //1/1
	pDescriptorOld->data[current_byte] += player.b_edit_strip << 3; //1/1
	
	pDescriptorOld->data[current_byte] += (player.boot_id << 4) & 240; //4/14
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.boot_id >> 4) & 255; //8/14
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.boot_id >> 12) & 3; //2/14
	
	pDescriptorOld->data[current_byte] += (player.glove_id << 2) & 252; //6/10
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.glove_id >> 6) & 15; //4/10
	
	//Unknown B - 4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.copy_id & 255; //8/32
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.copy_id >> 8) & 255; //8/32
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.copy_id >> 16) & 255; //8/32
	current_byte++;
	pDescriptorOld->data[current_byte] = (player.copy_id >> 24) & 255; //8/32
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.neck_len; //4/4
	
	pDescriptorOld->data[current_byte] += player.neck_size << 4; //4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.shldr_hi; //4/4
	
	pDescriptorOld->data[current_byte] += player.shldr_wid << 4; //4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.chest; //4/4
	
	pDescriptorOld->data[current_byte] += player.waist << 4; //4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.arm_size; //4/4
	
	pDescriptorOld->data[current_byte] += player.arm_len << 4; //4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.thigh; //4/4
	
	pDescriptorOld->data[current_byte] += player.calf << 4; //4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.leg_len; //4/4
	
	pDescriptorOld->data[current_byte] += player.head_len << 4; //4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.head_wid; //4/4
	
	pDescriptorOld->data[current_byte] += player.head_dep << 4; //4/4
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.wrist_col_l; //3/3
	pDescriptorOld->data[current_byte] += player.wrist_col_r << 3; //3/3
	pDescriptorOld->data[current_byte] += player.wrist_tape << 6; //2/2
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.spec_col; //3/3
	pDescriptorOld->data[current_byte] += player.spec_style << 3; //3/3
	pDescriptorOld->data[current_byte] += player.sleeve << 6; //2/2
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.inners; //2/2
	pDescriptorOld->data[current_byte] += player.socks << 2; //2/2
	pDescriptorOld->data[current_byte] += player.undershorts << 4; //2/2
	pDescriptorOld->data[current_byte] += player.tucked << 6; //1/1
	pDescriptorOld->data[current_byte] += player.ankle_tape << 7; //1/1
	current_byte++;
	
	pDescriptorOld->data[current_byte] = player.gloves; //1/1
	pDescriptorOld->data[current_byte] += player.gloves_col << 1; //3/3
	//Unknown D - 4/4
	current_byte++;
	...
	pDescriptorOld->data[current_byte] = player.skin_col; //3/3
	//Unknown F - 5/5
	current_byte++;
	...
	pDescriptorOld->data[current_byte] = player.iris_col; //4/4
	//Unknown H - 4/4
	current_byte++;
...
void extract_team_info17(team_entry team, int &current_byte, void* ghdescriptor)
{
	...
	current_byte+=0xE; //12
	pDescriptorOld->data[current_byte] = team.color1_red;

	pDescriptorOld->data[current_byte] += (team.color1_green << 6);
	current_byte++; //13
	pDescriptorOld->data[current_byte] = (pDescriptorOld->data[current_byte] & (~15)) | (team.color1_green >> 2);
	
	current_byte++; //14
	pDescriptorOld->data[current_byte] = team.color2_red;
	
	pDescriptorOld->data[current_byte] += (team.color2_green << 6);
	current_byte++; //15
	pDescriptorOld->data[current_byte] = (team.color2_green >> 2);
	
	pDescriptorOld->data[current_byte] += (team.color2_blue << 4);
	current_byte++; //16
	pDescriptorOld->data[current_byte] = (team.color2_blue >> 4);

	pDescriptorOld->data[current_byte] += (team.color1_blue << 2) & 255;
	
	current_byte+=0x3; //19
	pDescriptorOld->data[current_byte] = pDescriptorOld->data[current_byte] | (team.b_edit_name << 4);
